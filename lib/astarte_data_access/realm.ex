...
defmodule Astarte.DataAccess.Realm do
  ...
  defp check_no_connected_devices(keyspace) do
    ...
    case Repo.fetch_one(query, prefix: keyspace) do
      ...
      {:ok, _} ->
        ...
        Logger.warning(
          "Realm deletion preconditions are not satisfied: :connected_devices_present.",
  ...
  defp create_simple_triggers_table(keyspace) do
    # TODO: use Ecto migrations
    query = """
  ...
  defp insert_realm_public_key(keyspace, public_key_pem) do
    ...
    KvStore.insert(value, prefix: keyspace, consistency: :each_quorum)
  end

  def insert_realm_astarte_schema_version(keyspace, realm_schema_version) do
    value = %{
      group: "astarte",
      key: "schema_version",
      value: realm_schema_version,
  ...
  defp get_public_key(keyspace) do
    ...
    KvStore.fetch_value(group, key, :string,
      prefix: keyspace,
      consistency: :quorum,
  ...
  defp get_datastream_maximum_storage_retention(keyspace) do
    ...
    case KvStore.fetch_value(group, key, :integer, prefix: keyspace) do
      {:ok, value} -> value
      _ -> nil
    end
  end
